...
public class Board_Bar : MonoBehaviour
{
    public enum BarType
    {
        Left,
    ...
    private void OnMouseDown() // Collider만 있으면 가능
    {
        if (!Hanoi_Tower.isSelected) // 선택 안된 상태일 때
        {
            Hanoi_Tower.isSelected = true;
            Hanoi_Tower.selectedDonut = PopDonut();
        }
        else // 선택된 상태일때
        {
            PushDonut(Hanoi_Tower.selectedDonut);
        }
